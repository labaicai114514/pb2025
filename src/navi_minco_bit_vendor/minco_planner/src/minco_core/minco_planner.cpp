...
bool MincoPlanner::ReplanLocal(const geometry_msgs::msg::PoseStamped & current_pose)
{
  ...
  if (record_perf) {
    perf.emplace();
    perf->stamp_ros = node_.lock()->now().seconds();
  ...
  header_msg.stamp = node_.lock()->now();
  ...
  if (state == PlanningState::HOT_START) {
    const double now = node_.lock()->now().seconds() + 0.005;  // small buffer
  ...
  auto opt_start_time = node_.lock()->now().seconds();
  double final_cost =
  ...
  if (!std::isfinite(final_cost)) {
    ...
    if (has_last_traj && isTrajSafe()) {
      if (!isTrajectoryTimeExpired(node_.lock()->now().seconds())) {
        std::cout << YELLOW
  ...
  auto opt_end_time = node_.lock()->now().seconds();
  ...
  if (visualizer_) {
    ...
    {
      std::lock_guard<std::mutex> path_lock(path_mutex_);
      astar_path_msg.header.stamp = node_.lock()->now();
  ...
  last_traj_ = opt_traj;
  last_traj_.start_WT = node_.lock()->now().seconds();
  has_last_traj_ = true;
  ...
  const geometry_msgs::msg::Pose & start_pose, const std::vector<Eigen::Vector3d> & new_path)
{
  ...
  double now = node_.lock()->now().seconds() + 0.005;
