...
class RobotSocketHandler(Namespace):
    def __init__(self, namespace):
        ...
        #   srv
        # ==========================================
        self.ammo_exchange_client = node.create_client(ExchangeAmmon, "/exchange_ammo")

        # ==========================================
        #   pubs
        # ==========================================
        self.chassis_cmd_pub = node.create_publisher(
            ChassisCmd, "/%s/robot_base/chassis_cmd" % (robot_name), 10
        )
        self.gimbal_cmd_pub = node.create_publisher(
            GimbalCmd, "/%s/robot_base/gimbal_cmd" % (robot_name), 10
        )
        self.shoot_cmd_pub = node.create_publisher(
            ShootCmd, "/%s/robot_base/shoot_cmd" % (robot_name), 10
        )
        # ==========================================
        #   subs
        # ==========================================
    ...
    def on_connect(self):
        ...
        with thread_lock:
            if info_thread is None:
                info_thread = socketio.start_background_task(
                    ros_info_thread, node, robot_name
                )
        ...
        if autoAim and self.auto_aim_cmd.tracking:
            ...
            if movement_pitch <= 0:
                self.gimbal_pitch = max(
                    self.gimbal_pitch + movement_pitch, self.pitch_lower_bound
                )
            else:
                self.gimbal_pitch = min(
                    self.gimbal_pitch + movement_pitch, self.pitch_upper_bound
                )
